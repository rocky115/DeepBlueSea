  ...
  def add(self, **tracked):
    ...
    Raises:
      ValueError: If forbidden_attrs ('datetime') are used as keys in tracked, because
        the same key is used by ag to add key of the same value, which might get overwritten
    ...
    for attr, val in tracked.items():
      ...
      if attr in forbidden_attrs:
        raise ValueError(f'Attribute {attr} must not be present in forbidden_attrs {forbidden_attrs}, as ag uses them internally!')
